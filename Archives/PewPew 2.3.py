...
Musiquejeu=pygame.mixer.Sound('Bit Rush.wav')                                   # Création des variables qui correspondent aux différents sons utilisés
    ...
    if CompteurBo!=0:
        ...
        if Action1.keysym=="Left" and Coordonees[0]>50:
            ...
            ZoneJeu.move("Joueur2", 20, 0)
                                                                                ...
                                                                                # Idem pour le joueur deux avec les touches ZQSD et Ctrl_L
...
# La fonction ComparaisonDegats rend possible la création et la destruction d'une barre de vie.
# la commande "find_overlapping" permet de détecter le passage d'un missile sur un vaisseau, et de par la suite détruire une barre de vie du vaisseau touché
    ...
    if NombreCollisionJoueur1 > 3 and AncienneCollision!=CollisionsJoueur1[3] and Degats1==3:
        ...
        ZoneJeu.delete(VieJoueurUn3)                                            # Dans chacun des cas suivants, si un joueur est touché, une de ses barre de vie est enlevée et le laser est détruit
        ZoneJeu.after_cancel(id2)
        ZoneJeu.delete("Laser2")
        DelaiTir2=0
        AncienneCollision=CollisionsJoueur1[3]
    ...
    if NombreCollisionJoueur2 > 3 and AncienneCollision!=CollisionsJoueur2[3] and Degats2==3:
        ...
        ZoneJeu.after_cancel(id1)                                               # Dans chacun des cas suivants, si un joueur est touché, une de ses barre de vie est enlevée et le laser est détruit
        ZoneJeu.delete("Laser1")
        DelaiTir1=0
        AncienneCollision=CollisionsJoueur2[3]
        Degats2=Degats2-1
...
# car ils sont respectivement crées avec 2 et 4 coordonnées. Pour palier à ce problème, on place des rectangles sous l'image des vaisseaux, et on détecte les collisions entre les lasers et les rectangles
        ...
        VieJoueurUn3= ZoneJeu.create_rectangle (999+(30*3),550,1019+(30*3),587, fill="blue")           # Les variables VisJoueur correspondent aux rectangles qui représentent la barre de vie du joueur
...
def TraductionAnglais():
    global Francais,Nom1,Nom2,VariablePseudo
    ...
    BoutonTraductionAnglais.config(text="Passer le jeu en français",command=TraductionFrancais)    # La fonction TraductionAnglais permet de passer le texte des boutons en Anglais
    ...
    if VariablePseudo==0:
        Nom1="Player 1"
        Nom2="Player 2"                                                         # Si nous sommes en Français, les pseudos par défaut sont Player 1 et Player 2

# Bouton traduction français

def TraductionFrancais():
    global Francais,BoutonPseudo1,BoutonPseudo2,Nom1,Nom2,VariablePseudo
    ...
    BoutonBo5.config(text="Match en cinq manches")                              # La fonction TraductionAnglais permet de passer le texte des boutons en Français
    BoutonPseudo1.config(text="Valider pseudo joueur 1")
    BoutonPseudo2.config(text="Valider pseudo joueur 2")

    if VariablePseudo==0:
        Nom1="Joueur 1"                                                         # Si nous sommes en Français, les pseudos par défaut sont Joueur 1 et Joueur 2
        Nom2="Joueur 2"
...
def Rejouer():
    ...
    if CompteurBo==1:
        ...
        BoutonRetour.destroy()                                                  # La fonction Rejouer permet simplement de relancer une partie en
...
def Menu():
    ...
    BoutonTuto=Button(ZoneJeu,text="Tutoriel",height=5,width=30,command=Tuto)
...
def Getpseudo1():
    ...
    global Nom1,Pseudo1,VariablePseudo
...
def Getpseudo2():
    ...
    global Nom2,Pseudo2,VariablePseudo
